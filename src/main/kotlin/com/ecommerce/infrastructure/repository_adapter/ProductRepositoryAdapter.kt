...
class ProductRepositoryAdapter(
    ...
    override fun getById(id: String): Product? {
        val model = this.productRepository.findFullById(id).orElse(null)
        return ProductEntityMapper.toDomain(model)
    }

    override fun getBasicById(id: String): Product? {
        val model = this.productRepository.findBasicById(id).orElse(null)
        return ProductEntityMapper.toDomain(model)
    }

    override fun getByName(name: String): Product? {
        val model = this.productRepository.findFullByName(name).orElse(null)
        return ProductEntityMapper.toDomain(model)
    }
    ...
    override fun getAll(filters: ProductFilters?): List<Product> {
        ...
        val models = this.productRepository.findBy(spec) { query: FluentQuery.FetchableFluentQuery<ProductModel> ->
